...
class CropImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        // Resolve update_status — accept 1/0, "yes"/"no", "true"/"false" (case-insensitive)
        $rawStatus = $row['Update Status'] ?? $row['update_status'] ?? 0;
        if (is_string($rawStatus)) {
            $updateStatus = in_array(strtolower(trim($rawStatus)), ['1', 'yes', 'true']) ? 1 : 0;
        } else {
            $updateStatus = (int) $rawStatus;
        }
        ...
        return null; // updateOrCreate handles persistence
    }
