        ...
        <div class="col-12">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 border-bottom pb-2 gap-1">
                <div class="items-center gap-3">
                    ...
                    <p class="text-sage-600 dark:text-sage-400 text-sm mb-1" style="color: #777777">Create a new request for
                        crops or varieties</p>
                </div>
        ...
        <div class="card-body ">
            ...
            @php
                ...
                if ($selectedAccessionId) {
                    $selectedAccession = \App\Models\Accession::with('crop')->find($selectedAccessionId);
                }
            @endphp

            <form
                action="{{ isset($seedRequest) && $seedRequest->id ? route('requests.update', $seedRequest->id) : route('requests.store') }}"
                method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    @if (in_array($role, ['super-admin', 'admin']))
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="request_through" id="request_through"
                                ...
                                <option value="1"
                                    {{ old('request_through', $seedRequest->request_through ?? '') == '1' ? 'selected' : '' }}>
                                    Mail
                                </option>

                                <option value="2"
                                    {{ old('request_through', $seedRequest->request_through ?? '') == '2' ? 'selected' : '' }}>
                                    Call
                                </option>
                                <option value="3"
                                    {{ old('request_through', $seedRequest->request_through ?? '') == '3' ? 'selected' : '' }}>
                                    Self
                                </option>
                                <option value="4"
                                    {{ old('request_through', $seedRequest->request_through ?? '') == '4' ? 'selected' : '' }}>
                                    Whatsapp
                                </option>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="user_id" class="form-select" id="userSelect" required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" data-emp-reporting="{{ $user->emp_reporting }}"
                        ...
                        <div class="co-md-12">
                            ...
                            <div id="userReportingCard" class="card border bg-light d-none">
                                ...
                                <div class="card-body py-2 small">
                                    <div class="row g-1">
                                        <div class="col-4"><span class="text-muted">Name:</span> <span
                                                id="req_rm_name">—</span></div>
                                        <div class="col-4"><span class="text-muted">Email:</span> <span
                                                id="req_rm_email">—</span></div>
                                        <div class="col-4"><span class="text-muted">Mobile:</span> <span
                                                id="req_rm_mobile">—</span></div>
                                        <div class="col-4"><span class="text-muted">Code:</span> <span
                                                id="req_rm_code">—</span></div>
                                        <div class="col-4"><span class="text-muted">Dept:</span> <span
                                                id="req_rm_dept">—</span></div>
                                        <div class="col-4"><span class="text-muted">Designation:</span> <span
                                                id="req_rm_desig">—</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4 mb-3">
                        <label for="selectpurpose" class="form-label">Select Purpose <span
                                class="text-danger">*</span></label>
                        <select name="purpose" id="selectpurpose" class="form-select @error('purpose') is-invalid @enderror"
                            required>
                            ...
                            <option value="For Institutional Collaboration"
                                {{ old('purpose', $seedRequest->purpose ?? '') == 'For Institutional Collaboration' ? 'selected' : '' }}>
                                For Institutional Collaboration</option>
                            <option value="For External Research Partner"
                                {{ old('purpose', $seedRequest->purpose ?? '') == 'For External Research Partner' ? 'selected' : '' }}>
                                For External Research Partner</option>
                        </select>
                        @error('purpose')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                            ...
                            <select name="crop_id[]" class="form-select cropSelect" required>
                                ...
                                @foreach ($crops as $crop)
                                    <option value="{{ $crop->id }}"
                                        {{ $selectedAccession && $selectedAccession->crop_id == $crop->id ? 'selected' : '' }}>
                ...
                <div id="accessionDetailsBox" class="card border bg-light mb-3 d-none">
                    <div class="card-header py-2 bg-white d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <a href="#" id="viewDetailsLink" target="_blank"
                                class="btn btn-sm btn-outline-primary py-0 px-2" style="display:none">
                    ...
                    <div class="card-body py-2">
                        <div class="row g-2 small">
                            <div class="col-md-3"><span class="text-muted">Accession Name:</span><br><strong
                                    id="acc_name">—</strong></div>
                            <div class="col-md-3"><span class="text-muted">Crop:</span><br><strong
                                    id="acc_crop">—</strong></div>
                            <div class="col-md-3"><span class="text-muted">Total Quantity:</span><br><strong
                                    id="acc_total_qty" class="text-info">—</strong></div>
                            <div class="col-md-3"><span class="text-muted">Available Qty (User):</span><br><strong
                                    id="acc_qty" class="text-success">—</strong></div>
                            <div class="col-md-3"><span class="text-muted">Unit:</span><br><strong
                                    id="acc_unit">—</strong></div>
                            <div class="col-md-3"><span class="text-muted">Barcode:</span><br><strong
                                    id="acc_barcode">—</strong></div>
                        </div>
                        {{-- Lots breakdown --}}
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="purpose_details" class="form-label">Purpose Details <span
                                class="text-danger">*</span></label>
                        <input type="text" name="purpose_details"
                            value="{{ old('purpose_details', $seedRequest->purpose_details ?? '') }}" id="purpose_details"
                            class="form-control">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" id="notes" rows="3" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $seedRequest->notes ?? '') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit"
                        class="btn btn-primary">{{ isset($seedRequest) ? 'Update Request' : 'Create Request' }}</button>
                </div>
            </form>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
                    ...
                    fetch(`/employee/${reportingEmpId}`)
                        ...
                        .then(m => {
                            ...
                            fetch(`/check-user?emp_code=${m.emp_code}`)
                                .then(r => r.json())
                                .then(u => {
                                    document.getElementById('rm_user_badge_req').innerHTML = u
                                        .exists ?
                ...
                if (e.target.classList.contains('cropSelect')) {
                    ...
                    fetch(`/get-accessions/${cropId}`)
                        ...
                        .then(data => {
                            ...
                            data.forEach(acc => {
                                options +=
                                    `<option value="${acc.id}">${acc.accession_number}</option>`;
                            });
                    ...
                    fetch(`/accession/${id}`)
                        ...
                        .then(d => {
                            ...
                            document.getElementById('acc_qty').textContent =
                                (d.quantity_show ?? d.quantity) ? (d.quantity_show ?? d.quantity) +
                                ' ' + (d.unit || '') : '—';
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                if (invalid) {
                    e.preventDefault();
                    alert(
                        'One or more rows have a request quantity that exceeds the total available stock. Please check the highlighted fields.');
                }
