...
@section('content')
        ...
        <div class="col-12">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 border-bottom pb-2 gap-1">
                <div class="items-center gap-3">
                    ...
                    <p class="text-sage-600 dark:text-sage-400 text-sm mb-1" style="color: #777777">View and manage germplasm
                        requests</p>
                </div>
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const centerTextPlugin = {
                            ...
                            afterDraw(chart) {
                                ...
                                const percent = ((maxValue / total) * 100).toFixed(1);
                                const categoryName = chart.data.labels[maxIndex];
                                ...
                                ctx.fillText(categoryName, x, y + 15);
                                    ...
                                    tooltip: {
                                        ...
                                        callbacks: {
                                            label: function(context) {
                                                ...
                                                return `${context.label}: ${value} Requests (${percent}%)`;
                                            }
                                        }
                                    }
                                }
    ...
    <li class="nav-item">
        <a class="nav-link {{ request('status') == 'pending' ? 'active' : '' }}"
            ...
            Pending Approval
            <span class="badge bg-warning">{{ $statusData['pending'] ?? 0 }}</span>
        </a>
    </li>

    <li class="nav-item">
        <a class="nav-link {{ request('status') == 'dispatch' ? 'active' : '' }}"
            href="{{ route('requests.index', ['status' => 'dispatch']) }}">
            Dispatch
            <span class="badge bg-primary">
                {{ ($statusData['approved'] ?? 0) + ($statusData['dispatched'] ?? 0) }}
            </span>
        </a>
    </li>
</ul>
...
.custom-tabs .nav-link.active {
    ...
    background: transparent !important;
    border: none;
    border-bottom: 3px solid #497f83;
    font-weight: 600;
}</style>
                                ...
                                <tbody>
                                    @foreach ($requests as $request)
                                        <tr>
                                            ...
                                            <td>{{ $request->approved_at ? $request->approved_at->format('d M Y h:i A') : '-' }}
                                            </td>
                                            <td></td>
                                            <td>
                                                ...
                                                @if ($request->status === 'pending')
                                                    ...
                                                    <form action="{{ route('requests.destroy', $request->id) }}"
                                                        method="POST" class="d-inline d-none"
    ...
    $authEmpId = \Illuminate\Support\Facades\DB::table(
        'core_employee',
    )
    ...
    $isManager =
        $authEmpId &&
        (string) $requesterUser->emp_reporting ===
            (string) $authEmpId;
}
$canApprove =
    $request->status == 'pending' &&
    ($authUser->hasRole(['super-admin', 'admin', 'manager']) ||
                                                            $isManager);
                                                @endphp
                                                ...
                                                @if ($request->status == 'approved')
                                                    <button class="btn btn-sm btn-primary invoiceBtn"
                                                        data-id="{{ $request->id }}"
                                                        data-number="{{ $request->request_number }}"
                                                        data-crop="{{ $request->crop->crop_name }}"
                                                        data-quantity="{{ $request->quantity }}"
                                            ...
                                            <td>
                                                ...
                                                @elseif($request->status === 'returned')
                                                    <span class="text-warning small"><i
                                                            class="ri-arrow-go-back-line me-1"></i>Returned</span>
                                                @else
                                            ...
                                            <td>
                                                @if ($request->status === 'dispatched' || $request->status === 'received')
                                                    <button class="btn btn-sm btn-info regenerationsBtn"
                                                        data-id="{{ $request->id }}"
                                                        data-number="{{ $request->request_number }}"
                                                        data-crop="{{ $request->crop?->crop_name ?? '' }}"
                                                        data-quantity="{{ $request->quantity }}"
            ...
            @else
                <div class="card">
                    <div class="card-body text-center py-5">
                        <p class="text-slate-500">No requests found. <a href="#" class="add-request-link">Create
                                one</a></p>
                    </div>
                </div>
            @endif
                            ...
                            <tbody>
                                @foreach ($requests as $request)
                                    <tr>
                                        ...
                                        <!-- Dispatch -->
                                        <td>{{ $request->dispatch_date ? \Carbon\Carbon::parse($request->dispatch_date)->format('d M Y') : '-' }}
                                        </td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.viewRequestBtn').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    document.getElementById('view_request_number').textContent = this.getAttribute(
                        'data-number');
                    document.getElementById('view_crop').textContent = this.getAttribute(
                        'data-crop');
                    document.getElementById('view_quantity').textContent = this.getAttribute(
                        'data-quantity') + ' ' + this.getAttribute('data-unit');
                    document.getElementById('view_requester').textContent = this.getAttribute(
                        'data-requester');
                    document.getElementById('view_email').textContent = this.getAttribute(
                        'data-email') || 'N/A';
                    document.getElementById('view_phone').textContent = this.getAttribute(
                        'data-phone') || 'N/A';
                    document.getElementById('view_purpose').textContent = this.getAttribute(
                        'data-purpose') || 'N/A';
                    document.getElementById('view_request_date').textContent = this.getAttribute(
                        'data-request-date');
                    document.getElementById('view_required_date').textContent = this.getAttribute(
                        'data-required-date') || 'N/A';
                    document.getElementById('view_notes').textContent = this.getAttribute(
                        'data-notes') || 'N/A';

                    const status = this.getAttribute('data-status');
                    let statusBadge = '';
                    if (status === 'pending') statusBadge =
                        '<span class="badge bg-warning">Pending</span>';
                    else if (status === 'approved') statusBadge =
                        '<span class="badge bg-success">Approved</span>';
                    else if (status === 'rejected') statusBadge =
                        '<span class="badge bg-danger">Rejected</span>';
            ...
            if (addBtn) {
                addBtn.addEventListener('click', function(e) {
                    ...
                    const existing = document.getElementById('requestForm').querySelector(
                        'input[name="_method"]');
            ...
            document.querySelectorAll('.receiveBtn').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.getElementById('receiveRequestNumber').textContent = this.dataset
                        .number;
                    document.getElementById('receiveForm').action = '/requests/' + this.dataset.id +
                        '/receive';
            ...
            document.querySelectorAll('.regenerationsBtn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    document.getElementById('retg_qty').textContent = (g.quantity || '0') + ' ' + (g
                        .unit || '');
            ...
            document.querySelectorAll('.invoiceBtn').forEach(btn => {
                ...
                btn.addEventListener('click', function() {
                    ...
                    document.getElementById('inv_quantity').innerText = this.dataset.quantity +
                        " " + this.dataset.unit;
            ...
            <div class="modal-content">
                ...
                <form id="requestForm" method="POST" action="{{ route('requests.store') }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="requester_name" class="form-label">Requester Name <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="request_date" class="form-label">Request Date <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="row">
                            <div id="statusRow" class="col-md-6 mb-3" style="display: none;">
                                ...
                                <select name="status" id="status" class="form-select"
                                    style="width: auto;height:auto;position: relative;top: auto;margin: auto;left: 0;">
                                ...
                                <table class="table table-bordered table-striped p-0">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <p class="mb-0" id="view_request_number"></p>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <th>Crop</th>
                                            <td>
                                                <p class="mb-0" id="view_crop"></p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Quantity</th>
                                            <td>
                                                <p class="mb-0" id="view_quantity"></p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Required Date</th>
                                            <td>
                                                <p class="mb-0" id="view_required_date"></p>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <table class="table table-bordered table-striped p-0">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <p class="mb-0" id="view_requester"></p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Email</th>
                                        ...
                                        <tr>
                                            <th>Purpose</th>
                                            <td>
                                                <p class="mb-0" id="view_purpose"></p>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Notes</th>
                                        ...
                                        <tr>
                                            <th>Request Date:</th>
                                            <td>
                                                <p class="mb-0" id="view_request_date"></p>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="card bg-light mb-3">
                            <div class="card-body py-2 small">
                                <div class="row g-1">
                                    <div class="col-6"><span class="text-muted">Crop:</span> <span
                                            id="ret_crop"></span></div>
                                    <div class="col-6"><span class="text-muted">Dispatched Qty:</span> <span
                                            id="ret_qty"></span></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="return_quantity" id="returnQtyInput" class="form-control"
                                step="0.01" min="0.01" required placeholder="Quantity being returned">
                        </div>
                        <div class="mb-3">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" id="receiveForm" action="#">
                    @csrf
                    <div class="modal-body">
                        <p>Mark request <strong id="receiveRequestNumber"></strong> as <span
                                class="badge bg-success">Received</span>?</p>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="card bg-light mb-3">
                            <div class="card-body py-2 small">
                                <div class="row g-1">
                                    <div class="col-6"><span class="text-muted">Crop:</span> <span
                                            id="retg_crop"></span></div>
                                    <div class="col-6"><span class="text-muted">Dispatched Qty:</span> <span
                                            id="retg_qty"></span></div>
                                </div>
                            </div>
                        </div>
                        <div class="row mt-2" id="partialReturnFields">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="number" name="germination_rate" class="form-control"
                                    placeholder="Enter germination rate">
                            </div>
                            <div class="mb-3 col-md-6">
                                <label class="form-label">Moisture Rate</label>
                                <input type="number" name="moisture_rate" class="form-control"
                                    placeholder="Enter moisture rate">
                            </div>
                        </div>
